...
def train(model: torch.nn.Module, 
    ...
    for epoch in tqdm(range(epochs)):
        ...
        torch.save(checkpoint, f"Epoch-{epoch+1}-checkpoint.pth.tar")
...
def main():
    ...
    optimizer = torch.optim.Adam(
        ...
        lr=0.0001339968154986823,
        weight_decay=0.008210241142662276
    )
    ...
    plt.plot(np.arange(len(results["train_loss"])), results["train_loss"], 'r')
    plt.title("Train loss")
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.show()

    plt.plot(np.arange(len(results["train_acc"])), results["train_acc"], 'r')
    plt.title("Train accuracy")
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.show()

    plt.plot(np.arange(len(results["test_loss"])), results["test_loss"], 'r')
    plt.title("Test loss")
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.show()

    plt.plot(np.arange(len(results["test_acc"])), results["test_acc"], 'r')
